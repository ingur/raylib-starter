...
def numeric_constants(api: Api) -> list[tuple[str, str, float]]:
    """(name, comment, value) for enums, colours and numeric defines."""
    out = []
    for enum in api.raw["enums"]:
        for v in enum["values"]:
            out.append((v["name"], enum["name"], float(v["value"])))
    for d in api.raw["defines"]:
        if d["type"] == "COLOR":
            r, g, b, a = (int(x) for x in re.search(r"\{\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)\s*\}", d["value"]).groups())
            out.append((d["name"], "Color", float((r << 24) | (g << 16) | (b << 8) | a)))
        elif d["type"] in ("INT", "FLOAT"):
            out.append((d["name"], "define", float(d["value"])))
        elif d["type"] == "FLOAT_MATH":
            expr = re.sub(r"(\d)f\b", r"\1", d["value"])
            out.append((d["name"], "define", float(eval(expr, {"PI": math.pi}))))  # noqa: S307
    return out
...
def emit_defs(api: Api, bound: list[tuple[dict, str]], version: str) -> str:
    out = [
        ...
        f"-- Point your editor at it: luau-lsp --definitions=types/raylib.d.lua\n\n"
    ]
    ...
    out.append("\n")
    for name, comment, _ in numeric_constants(api):
