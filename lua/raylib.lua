...
ffi.cdef[[
    ...
    typedef void (*AudioCallback)(void *bufferData, unsigned int frames); // From raylib.h
...
raylib = ffi.load("raylib")

rl = {}

setmetatable(rl, { __index = raylib })
...
function rl.EaseExpoInOut(t, b, c, d)
  ...
  t = t / (d / 2.0)
  if t < 1.0 then
    return c / 2.0 * pow(2.0, 10.0 * (t - 1.0)) + b
  else
...
function rl.EaseElasticIn(t, b, c, d)
  ...
  local a = c
  local s = p / 4.0
  local postFix = a * pow(2.0, 10.0 * t)
...
function require(modname)
    ...
    local file_path = "lua/" .. modname:gsub("%.", "/") .. ".lua"
    local content = rl.LoadFileText(file_path)

    if not content then
        error("module '" .. modname .. "' not found in path '" .. file_path .. "'")
    end
    
    local chunk, err = loadstring(ffi.string(content), "@" .. file_path)
    rl.UnloadFileText(content)

    if not chunk then
        error("error loading module '" .. modname .. "' from file '" .. file_path .. "': " .. err)
    end
